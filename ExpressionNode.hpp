...
public:
    ArrayAccessNode(Node* array, Node* index, int l)
        : Node("Array access", l, { array, index })
        , array { array }
        , index { index } {};
...
public:
    ArrayLengthNode(Node* array, int l)
        : UnaryExpressionNode("Array length", array, l)
        , array { array } {};
    std::string checkTypes(SymbolTable& st) const override;
};
...
public:
    NotNode(Node* expr, int l)
        : UnaryExpressionNode("Negated expression", expr, l)
        , expr { expr } {};
    std::string checkTypes(SymbolTable& st) const override;
};
...
public:
    ClassAllocationNode(Node* object, int l)
        : Node("Class allocation", object->value, l)
        , id { object->value } {};
    std::string checkTypes(SymbolTable& st) const override;
};
