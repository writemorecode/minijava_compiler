...
std::string IntegerArrayAllocationNode::checkTypes(SymbolTable& st) const
{
    ...
    if (lengthType != "int") {
        std::cerr << "Error: (line " << lineno << ") Invalid type '" << lengthType
                  << "' for array length"
                     ", expected type 'int'.\n";
        return "";
    }
...
std::string ArrayLengthNode::checkTypes(SymbolTable& st) const
{
    const auto arrayType = array->checkTypes(st);
    if (arrayType != "int[]") {
        std::cerr << "Error: (line " << lineno << ") Invalid type '" << arrayType
                  << "' for array 'length', "
                     "expected type 'int[]'.\n";
        return "";
    }
...
std::string EqualToNode::checkTypes(SymbolTable& st) const
{
    ...
    if ((lhsType == "boolean" && rhsType == "boolean") || (lhsType == "int" && rhsType == "int")) {
        return "boolean";
    }
    std::cerr << "Error: ";
    std::cerr << "(line " << lineno << ") ";
